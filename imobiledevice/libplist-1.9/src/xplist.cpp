...
static void node_to_xml(node_t* node, void *xml_struct)
{
    ...
    case PLIST_UINT:
        tag = XPLIST_INT;
        val = (char*)malloc(64);
        (void)_snprintf(val, 64, "%lld", node_data->intval);
        break;
...
static void xml_to_node(xmlNodePtr xml_node, plist_t * plist_node)
{
    ...
    for (node = xml_node->children; node; node = node->next)
    {
        ...
        if (*plist_node)
            node_attach((struct node_t*)*plist_node, (struct node_t*)subnode);
        else
            *plist_node = subnode;
        ...
        if (!xmlStrcmp(node->name, XPLIST_INT))
        {
            ...
            data->intval = strtoul((char*)strval, NULL, 0);
            data->type = PLIST_UINT;
            data->length = 8;
            xmlFree(strval);
            continue;
...
void plist_to_xml(plist_t plist, char **plist_xml, uint32_t * length)
{
    ...
    node_to_xml((struct node_t*)plist, (void*)&root);
