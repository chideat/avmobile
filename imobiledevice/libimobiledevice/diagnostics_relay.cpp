...
diagnostics_relay_error_t diagnostics_relay_query_ioregistry_entry(diagnostics_relay_client_t client, const char* name, const char* cla, plist_t* result)
{
	if (!client || (name == NULL && cla == NULL) || result == NULL)
		return DIAGNOSTICS_RELAY_E_INVALID_ARG;
	...
	if (cla)
		plist_dict_insert_item(dict,"EntryClass", plist_new_string(cla));
